...
def clean_311():
    ...
    with open(file_input, newline='') as csvfile:
        ...
        for row in reader:
            ...
            tuple_out = Encampment(row.get('CaseID'), date_year, date_month, datetime_object, float(row.get('Latitude')),
                                   float(row.get('Longitude')), row.get('Neighborhood'), row.get('Status Notes').lower())
