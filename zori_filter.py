...
def get_cleaned_zori_dict() :
    ...
    # filter month-year(2019-2023)
    date_cols = [col for col in zip_sf.columns if any(yr in col for yr in ['2019', '2020', '2021', '2022', '2023'])]
    ...
    #make dic
    rent_dict = melted.dropna(subset=['rent']).groupby('month_year').apply(
    lambda x: list(zip(x['zip'], x['rent']))
    ).to_dict()
